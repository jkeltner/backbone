...
Once per episode, after all three review meetings have happened, this command reads every checkpoint feedback file together — both solo notes (`0N-jeff-notes.md`, `0N-cyrus-notes.md`) and the audio review transcripts (`01-blueprint.txt`, `02-script.txt`, `03-polish.txt`) — and proposes targeted edits to `roles/` and `hosts/` files — the kind of fixes that prevent the same issue recurring on future episodes.
...
Verify that at least one checkpoint feedback file exists at `episodes/$ARGUMENTS/feedback/`. The full expected set (any subset is acceptable):
- Checkpoint 1: `01-jeff-notes.md`, `01-cyrus-notes.md`, `01-blueprint.txt`
- Checkpoint 2: `02-jeff-notes.md`, `02-cyrus-notes.md`, `02-script.txt`
- Checkpoint 3: `03-jeff-notes.md`, `03-cyrus-notes.md`, `03-polish.txt`

If none exist, stop and tell the user to save at least one feedback file first. If only some exist, proceed with what's there and note which were missing in the proposals file.

## Run the audit

Spawn a general-purpose agent with:

- **Task:** "Audit the Backbone pipeline's role and host files against the full set of checkpoint feedback for the topic **$ARGUMENTS** — both solo notes and audio review transcripts. Look for cross-checkpoint patterns, not just per-meeting issues. Propose targeted edits that would prevent the same issues recurring on future episodes. Do NOT edit any files directly — produce a proposals document only."
- **Files to read:**
  - All available feedback files in `episodes/$ARGUMENTS/feedback/` (skip any that don't exist; note their absence):
    - Solo notes: `01-jeff-notes.md`, `01-cyrus-notes.md`, `02-jeff-notes.md`, `02-cyrus-notes.md`, `03-jeff-notes.md`, `03-cyrus-notes.md`
    - Audio transcripts: `01-blueprint.txt`, `02-script.txt`, `03-polish.txt`
...
- **Methodology:**
  1. Read all available feedback (solo notes + transcripts, all checkpoints) end-to-end before proposing anything. Get the full picture first.
  ...
  4. **Solo-notes vs. transcript divergence is itself signal.** If Jeff or Cyrus flagged something in their solo notes but it didn't survive the audio review, that's worth noting — sometimes the conversation resolved it (no action), sometimes it got dropped under time pressure (worth proposing anyway). Use judgment.
  5. For each, decide whether it's:
  ...
  6. Stay in scope: only propose edits to `roles/*.md` and `hosts/*.md`. CLAUDE.md and template changes are out of scope and go through `/pipeline-review` post-episode — note them in a "Out of scope — defer to /pipeline-review" section instead.
  7. For each proposed edit, include: target file, the existing text being changed (with enough context to locate it), the proposed replacement, and a 1–2 sentence rationale citing the specific feedback (which meeting/notes file, what was said).
...
date: {YYYY-MM-DD}
sources:
  # mark each missing if absent
  - episodes/$ARGUMENTS/feedback/01-jeff-notes.md
  - episodes/$ARGUMENTS/feedback/01-cyrus-notes.md
  - episodes/$ARGUMENTS/feedback/01-blueprint.txt
  - episodes/$ARGUMENTS/feedback/02-jeff-notes.md
  - episodes/$ARGUMENTS/feedback/02-cyrus-notes.md
  - episodes/$ARGUMENTS/feedback/02-script.txt
  - episodes/$ARGUMENTS/feedback/03-jeff-notes.md
  - episodes/$ARGUMENTS/feedback/03-cyrus-notes.md
